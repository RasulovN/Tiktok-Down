...
def start(client, message):
    kb = [[InlineKeyboardButton('Channel 🛡', url=chnnl),InlineKeyboardButton('Channel 2 🔰', url="https://t.me/new_states")]]
    reply_markup = InlineKeyboardMarkup(kb)
    app.send_message(chat_id=message.from_user.id, text=f"Hello, I'm **TikTok Downloader Botman**.\nI can download TikTok video without Watermark.\n\n"
                          "__**Developer :**__ __@rasulov_n7__\n"
...
def help(client, message):
    kb = [[InlineKeyboardButton('Channel 🛡', url=chnnl),InlineKeyboardButton('Repo 🔰', url="https://github.com/RasulovN/Tiktok-Down/tree/main/TikTok-DownloaderPro-Bot")]]
    reply_markup = InlineKeyboardMarkup(kb)
...
def tiktok_dl(client, message):
    a = app.send_message(chat_id=message.chat.id,
                         text='__The file is being downloaded from the server...__',
                         parse_mode='md')
    ...
    with requests.get(r, timeout=(50, 10000), stream=True) as r:
        ...
        a.edit(f'__Downloaded to the server!\n'
               f'Hozir Telegramga yuklanmoqda ⏳__')
        start = time.time()
        title = filename
